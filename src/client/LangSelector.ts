...
import zh from "../../resources/lang/zh-CN.json";
...
export class LangSelector extends LitElement {
  ...
  // Langues avec pages dédiées (ajout de 'zh')
  private seoLanguages = ['en', 'fr', 'de', 'es', 'it', 'pt', 'ru', 'pt-br', 'nl', 'pl', 'tr', 'ko', 'ja', 'zh'];
  ...
  private async initializeLanguage() {
    // Priorité : 1. localStorage, 2. URL, 3. navigateur
    const savedLang = localStorage.getItem("lang");
    const urlLang = this.getLanguageFromUrl();
    const browserLocale = navigator.language;
    
    // Utiliser la langue sauvegardée si elle existe, sinon celle de l'URL, sinon celle du navigateur
    const userLang = this.getClosestSupportedLang(savedLang || urlLang || browserLocale);
  ...
  private changeLanguage(lang: string) {
    ...
    // Navigation SEO
    // Si on change vers une langue qui a une page dédiée ET qu'on n'est pas déjà sur cette page
    if (this.seoLanguages.includes(lang)) {
      const currentPath = window.location.pathname;
      const currentLangInUrl = this.getLanguageFromUrl();
      
      // Construire le nouveau chemin
      let newPath = '';
      const pathParts = currentPath.split('/').filter(p => p);
      
      // Retirer la langue actuelle si elle existe
      if (currentLangInUrl && pathParts[0] === currentLangInUrl) {
        pathParts.shift();
      }
      ...
      // Naviguer vers la nouvelle URL si elle est différente
      if (newPath !== currentPath) {
        window.location.href = newPath;
      }
    } else {
      // Pour les langues sans page dédiée, on reste sur la page actuelle
      // mais on met à jour la traduction
      // Si on est sur une page de langue spécifique, on retourne à la racine
      const currentLangInUrl = this.getLanguageFromUrl();
      if (currentLangInUrl && this.seoLanguages.includes(currentLangInUrl)) {
        const pathParts = window.location.pathname.split('/').filter(p => p);
        pathParts.shift(); // Retirer la langue
